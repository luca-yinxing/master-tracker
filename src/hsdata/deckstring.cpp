...
write_deckstring_page(const Deck& src, std::string& dest) noexcept
{
	...
	constexpr const size_t maxlen = DEF_BUFFER_STR_LONG;
	char buffer[maxlen];
	...
	tmp += snprintf(tmp,
			maxlen - (tmp - buffer),
			"# Class: %s\n",
			conv_class_string_en(src.hero_class()));

	tmp += snprintf(tmp,
			maxlen - (tmp - buffer),
			"# Format: %s\n",
			conv_format_string(src.format()));

	tmp += snprintf(tmp, maxlen - (tmp - buffer), "# \n");
	...
	tmp += snprintf(tmp, maxlen - (tmp - buffer), "# \n");
	tmp += snprintf(tmp, maxlen - (tmp - buffer), "%s\n", code.c_str());
	tmp += snprintf(tmp, maxlen - (tmp - buffer), "# \n");
	tmp += snprintf(
	    tmp,
	    maxlen - (tmp - buffer),
	...
	tmp += snprintf(tmp,
			maxlen - (tmp - buffer),
			"# Generated by Master Tracker - HSData\n");
	return EXE_OK;
}
