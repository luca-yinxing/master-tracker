...
void
PlayWindow::init(const std::shared_ptr<GameManager>& gm)
{
	if (gm == nullptr) {
		return;
	}
	...
	connect(gamemanager.get(),
		&GameManager::user_changed,
		this,
		&PlayWindow::player_cards_changed);

	return;
}

void
PlayWindow::player_cards_changed(const std::shared_ptr<Player>& user)
{
	if (user == nullptr) {
